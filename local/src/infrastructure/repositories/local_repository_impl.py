...
class LocalRepositoryImpl(LocalRepository):
    ...
    async def get_by_bairro(self, bairro: str) -> List[Local]:
        ...
        result = await self.session.execute(
            select(LocalModel).where(LocalModel.bairro == bairro)
        )
        models = result.scalars().all()
        return [self._model_to_entity(model) for model in models]

    async def count(self) -> int:
        """Conta o total de locais no banco de dados"""
        result = await self.session.execute(select(func.count()).select_from(LocalModel))
