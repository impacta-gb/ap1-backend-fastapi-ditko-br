...
from loca.src.application.schemas.local_schema import (
    LocalCreate,
    LocalResponse,
    LocalUpdate,
)
...
from local.src.domain.entities.item import Local
from local.src.infrastructure.database.config import get_session
from localsrc.infrastructure.repositories.item_repository_impl import LocalRepositoryImpl

router = APIRouter(prefix="/locals", tags=["Locals"])

@router.post("/locals", response_model=LocalResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/locals/bairro/{bairro}", response_model=List[LocalResponse])
...
@router.put("/locals/{local}", response_model=LocalResponse)
