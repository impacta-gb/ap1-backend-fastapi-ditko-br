"""Bootstrap local de mensageria do módulo Item."""

import asyncio
import logging

from src.infrastructure.messaging.producer import ItemKafkaProducer
...
class MessagingBootstrap:
    """Gerencia startup/shutdown de producer e consumers do módulo Item."""
    ...
    def __init__(self):
        ...
        self.local_event_consumer = LocalEventConsumer()
    ...
    async def start_producers(self):
        producers = [(self.item_producer, "Item")]
        for producer, name in producers:
            await self._start_component(producer, name, "Producer")

    async def start_consumers(self):
        consumers = [
            (self.devolucao_event_consumer, "Item Devolucao Event"),
            (self.local_event_consumer, "Item Local Event"),
            (self.responsavel_event_consumer, "Item Responsavel Event"),
        ]
