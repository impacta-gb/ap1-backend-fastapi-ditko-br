"""
Consumer base class para Kafka - Reclamante
"""
import json
import logging
import os
    ...
    async def stop(self):
        ...
        if self.consumer:
            try:
                ...
                logger.error(f"Erro ao parar {self.__class__.__name__}: {e}")
    
    async def _process_messages(self):
        """Processa mensagens do Kafka em loop"""
        if not self.consumer:
            logger.warning(f"{self.__class__.__name__} não conseguiu se conectar ao Kafka")
            return
        ...
        try:
            ...
            logger.error(f"Erro ao processar mensagem em {self.__class__.__name__}: {e}")
    
    @abstractmethod
    async def handle_message(self, message: dict):
        """Método abstrato para processar a mensagem
