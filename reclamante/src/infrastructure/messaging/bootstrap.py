"""Bootstrap local de mensageria do módulo Reclamante."""

import asyncio
import logging

from src.infrastructure.messaging.producer import ReclamanteKafkaProducer
from src.infrastructure.messaging.consumers import (
    ItemEventConsumer,
    ResponsavelEventConsumer,
...
class MessagingBootstrap:
    """Gerencia startup/shutdown de producer e consumers do módulo Reclamante."""
    ...
    def __init__(self):
        ...
        self.responsavel_event_consumer = ResponsavelEventConsumer()
    ...
    async def start_producers(self):
        producers = [(self.reclamante_producer, "Reclamante")]
        for producer, name in producers:
            await self._start_component(producer, name, "Producer")

    async def start_consumers(self):
        consumers = [
            (self.item_event_consumer, "Reclamante Item Event"),
            (self.responsavel_event_consumer, "Reclamante Responsavel Event"),
            (self.devolucao_event_consumer, "Reclamante Devolucao Event"),
        ]
