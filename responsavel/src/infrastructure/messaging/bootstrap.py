"""Bootstrap local de mensageria do módulo Responsável."""

import asyncio
import logging

from src.infrastructure.messaging.producer import ResponsavelKafkaProducer
...
class MessagingBootstrap:
    """Gerencia startup/shutdown de producer e consumers do módulo Responsável."""
    ...
    async def start_producers(self):
        producers = [(self.responsavel_producer, "Responsavel")]
        for producer, name in producers:
            await self._start_component(producer, name, "Producer")

    async def start_consumers(self):
        consumers = [(self.item_event_consumer, "Responsavel Item Event")]
